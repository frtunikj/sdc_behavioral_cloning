...
def modifiedNVidiaCNNModel():
    # Difference to the Nvidia Model is the cropping and the dropout layers.
    model = Sequential()
    model.add(Cropping2D(cropping=((70, 25), (0, 0)), input_shape = (160, 320, 3)))
    # normalize data.
    model.add(Lambda(lambda x: (x / 255) - 0.5, input_shape=(160,320,3)))
    model.add(Convolution2D(24,5,5, subsample=(2,2), activation='relu', border_mode='valid', W_regularizer=l2(0.001)))
    model.add(Convolution2D(36,5,5, subsample=(2,2), activation='relu', border_mode='valid', W_regularizer=l2(0.001)))
    model.add(Convolution2D(48,5,5, subsample=(2,2), activation='relu', border_mode='valid', W_regularizer=l2(0.001)))
    model.add(Convolution2D(64,3,3, activation='relu', border_mode='valid', W_regularizer=l2(0.001)))
    model.add(Convolution2D(64,3,3, activation='relu', border_mode='valid', W_regularizer=l2(0.001)))
    model.add(Flatten())
...
history_object = model.fit_generator(training_generator, samples_per_epoch= \
                 ...
                 nb_val_samples=2*len(validation_samples), nb_epoch=6, verbose=1)
